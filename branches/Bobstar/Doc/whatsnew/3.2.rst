...
这篇文章解释了在Python 3.2相比3.1的新功能，
它着重于几个亮点，并给出了几个例子。对于
详细内容，见Misc/NEWS。
...
在过去，一个Python版本生成的扩展模块往往
不能用于其他Python版本。特别是在Windows中，每个
版本的Python都会重建所依赖的扩展模块。
这是因为要获得自由链接到Python解释器而专门规定的，这样才能使得那些扩展模块可以被使用。
...
随着新的Python 3.2，另一种解决方法已可用：
扩展模块，通过将自己restict到一个有限制的API（定义
Py_LIMITED_API）虽然只能使用部分内部API，
但这被认为是一个可以建立稳定跨版本的API集合。
因此，扩展模块这种创建模式在
Python版本3.3，3.4，等等也可以被使用。
那些关于内存结构细节的扩展模块会被自动创建
同时每个版本的特色功能部分将会被重新编译。
...
*Python优化建议 389：Argparse命令行解析模块
=============================================
...
一个用于命令行解析新模块，`` argparse ``，它被用来
解决``optparse ``的局限性，后者没有提供支持
自定义参数（不只是选项），子命令，要求的选择
和其他普通的指定和验证模式。
...
该模块在开发社区中作为第三方模块已经获得广泛的成功。
比其前任更丰富的特性，
``argparse`` 模块是目前命令行处理的首选模块。
旧的模块仍被保留继续使用，因为
有大量的遗留代码依赖于它。
...
这里有一个例子分析器，显示那些可选的特性功能集合，
并指定一个* metavar *在
屏幕帮助界面上，验证一个或多个位置参数是否存在，
并生成所需的选项::
...
一个argparse极好的的功能是能够定义subparsers，
subparsers，每次解析到自定义的参数都会显示相应的帮助文档::
...
"logging"模块提供两种配置方式，第一种
针对每个选项都有一个函数来处理，另一种在
外部保存一个`` ConfigParser ``格式文件。这些办法
不能从JSON或YAML文件灵活的创建配置项目，
也不支持增量配置，
对于特定的命令行日志记录器这是必须的。
...
为了支持更灵活的风格，现在提供一个专用模块
`` logging.config.dictConfig（）``用于
使用普通的Python字典创建日志记录项。配置选项包括
格式化，处理器，过滤器和记录器。这里有一个关于字典配置日志的实例：:
...
*Python优化建议 3148：``concurrent.futures`` 模块
=================================================================
...
创建和管理并发的代码被收集存放在一个新的
顶级命名空间*concurrent*。第一个成员是*futures*包，
它提供了一个统一管理线程和进程的高层次的接口。
...
`` concurrent.futures``的设计灵感来自
* java.util.concurrent.package *.在此模式下，调用代码和运行结果都
是表现为一个``Future``对象，摘要功能
常见的线程，进程和远程过程调用。该对象
支持状态检查（跑步或做），超时，取消，
加入回调，并获得结果或例外。
...
这个新模块的首要特色是有一对运行和管理代码调用的执行器类。
该执行者器的目标是
让使用现有的工具进行并行调用更加容易。他们
需要设置一个资源池来保留这种效果，而后运行代码调用，
创建结果队列，添加超时处理器，并限制
线程，进程或远程调用总数。
...
这两个类共享一个通用的接口，包含三个方法：``submit（）``
调度一个可调用代码并返回一个``Future``对象; ``map（）``
调度在当时多个异步调用，``shutdown()``
用来释放资源。这个类是一个*context manager*，
可以被用在``with``声明中，以确保当前pending futures运行完毕时资源被
自动释放。
...
*Python优化建议 3333：Python的Web服务器网关接口1.0.1
=====================================================
