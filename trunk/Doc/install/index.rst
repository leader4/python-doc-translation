...
虽然Python的标准库广泛涵盖了许多的编程需求,
但经常,你需要增加一些新功能,通过
Python安装第三方模块的形式. 这可能是必要的步骤以
支持您自己的代码,或支持一个应用程序. 

在过去,支持当前Python安装包的第三方模块比较少. 随着
在Python 2.0中引进Python Distribution Utilitiies (简化版的Distutils) ,情况才大为改变. 

本文件主要针对的是需要安装第三方
Python模块的人: 最终用户和系统管理员只需要得到一些
Python应用程序运行,现有的Python程序员,想添加一些
新的好东西到他们的工具箱. 你并不需要多么了解Python才来阅读本
文件;会有一些简短的介绍如何使用Python的交互模式
探索您的安装包,但仅此而已. 如果你正在寻找的信息
包括如何发布你自己开发的Python模块,让其他人可以使用它们,请参阅
:ref:`distutils-index`. 
...
在最好的情况下,有人会已经准备了一个特殊的模块版本
你要安装的分布,特别是针对你的平台
就像任何其他软件在你的平台上安装. 例如,
模块的开发可能使一个可执行安装程序可用于Windows
用户,基于RPM的Linux系统的用户 (红帽,SUSE的RPM包,
Mandrake的,和其他许多人) ,一个基于Debian的Linux用户的Debian软件包
系统,等等. 

在这种情况下,你会下载安装适合您的平台
用它做显而易见的事情: 运行它,如果它是一个可执行安装程序,`` RPM
- 安装 ",如果它的RPM包,等你并不需要运行Python或设置
脚本,你不需要编译任何东西---你可能甚至不需要阅读任何
说明 (虽然它总是一个好主意,所以不管怎么说) . 

当然,事情并不总是那么容易. 你可能会在感兴趣
模块分布不具有一个易于使用的安装程序,为您的
平台. 在这种情况下,你必须先从源分布
公布的模块的作者/维护者. 从源头上安装
的分布是不太难,只要这些模块打包在
标准的方法. 这份文件的大部分是关于建设和安装
从标准的源代码分发的模块. 
...
如果您下载一个模块的源分布,你可以很快告诉
打包和分发标准的方式,即使用的Distutils. 
首先,分配的名称和版本号将突出
在下载的档案,例如名称: 文件: `FOO -1.0.tar.gz`
: 文件: `部件0.9.7.zip`. 接下来,将其解压缩到一个同样名为存档
目录: 文件: `foo- 1.0的`: 文件: `小部件0.9.7`. 此外,
分布将包含安装脚本文件: `setup.py`,和一个名为
: 文件的`readme.txt`或可能只是: 文件: '自述',这应该解释
建设和安装的模块分配是运行一个简单的事情: 

   python setup.py install

如果所有这些东西都是真实的,那么你已经知道如何构建和安装
您刚刚下载的模块: 运行上面的命令. 除非你需要
安装在一个非标准的方式东西,或自定义构建过程中,你不
真正需要本手册. 或者说,上面的命令是你需要的一切
走出本手册. 
...
备用安装方法: Windows (the prefix scheme)
---------------------------------------------------
...
使用 non-Microsoft 编译器 on Windows
----------------------------------------
