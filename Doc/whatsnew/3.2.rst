...
这篇文章解释了在Python 3.2.2相比3.1的新功能,它着重于几个亮点,并给出了几个例子. 对于详细内容. 
...
PEP 384: 定义了一个稳定的 ABI  (PEP: Python社区提交讨论的Python优化建议) 
======================================================================

在过去,一个Python版本生成的扩展模块往往不能用于其他Python版本. 特别是在Windows中,每个版本的Python都会重建所依赖的扩展模块. 
这是因为要获得自由访问到相应的Python解释器而专门规定的,只有这样才能使得那些扩展模块可以被使用. 
...
随着新的Python 3.2,另一种解决方法已出现: 
扩展模块,通过将自己restict到一个有限制的API (定义Py_LIMITED_API) 虽然只能使用部分内部API,但这被认为是一个可以建立稳定跨版本的API集合. 
因此,扩展模块这种创建模式在Python版本3.3,3.4,等等也可以被使用. 
那些关于内存结构细节的扩展模块会被自动创建同时每个版本的特色功能部分将会被重新编译. 
...
*Python PEP 389: Argparse命令行解析模块
=============================================
...
一个用于命令行解析新模块,`` argparse ``,它被用来解决``optparse ``的局限性,后者没有提供支持
自定义参数 (不只是选项) ,子命令,要求的选择和其他普通的指定和验证模式. 
...
该模块在开发社区中作为第三方模块已经获得广泛的成功. 比其前任更丰富的特性,``argparse`` 模块是目前命令行处理的首选模块. 
旧的模块仍被保留继续使用,因为有大量的遗留代码依赖于它. 
...
这里有一个例子分析器,显示那些可选的特性功能集合,并指定一个* metavar *在屏幕帮助界面上,验证一个或多个位置参数是否存在,并生成所需的选项::
...
一个argparse极好的的功能是能够定义subparsers,subparsers,每次解析到自定义的参数都会显示相应的帮助文档::
...
"logging"模块提供两种配置方式,第一种针对每个选项都有一个函数来处理,另一种在外部保存一个`` ConfigParser ``格式文件. 这些办法
不能从JSON或YAML文件灵活的创建配置项目,也不支持增量配置,对于特定的命令行日志记录器这是必须的. 
...
为了支持更灵活的风格,现在提供一个专用模块`` logging.config.dictConfig () ``用于
使用普通的Python字典创建日志记录项. 配置选项包括格式化,处理器,过滤器和记录器. 这里有一个关于字典配置日志的实例: :
...
如果该字典是存储在`` conf.json ``文件中,它可以像这样被加载和调用::
...
*Python PEP 3148: concurrent.futures 模块
=================================================================
...
创建和管理并发的代码被收集存放在一个新的顶级命名空间*concurrent*. 第一个成员是*futures*包,它提供了一个统一管理线程和进程的高层次的接口. 
...
concurrent.futures的设计灵感来自* java.util.concurrent.package *.在此模式下,调用代码和运行结果都是表现为一个``Future``对象,摘要功能
常见的线程,进程和远程过程调用. 该对象支持状态检查 (跑步或做) ,超时,取消,加入回调,并获得结果或例外. 
...
这个新模块的首要特色是有一对运行和管理代码调用的执行器类. 该执行者器的目标是让使用现有的工具进行并行调用更加容易. 他们
需要设置一个资源池来保留这种效果,而后运行代码调用,创建结果队列,添加超时处理器,并限制线程,进程或远程调用总数. 
...
理想的情况下,每个应用程序独享一个链接多个组件的执行器,以便对进程和线程的各种限制进行集中管理. 这解决了设计时出现的缺陷,每个组件都有自己的资源管理器这种竞争机制. 
...
这两个类共享一个通用的接口,包含三个方法: ``submit () ``调度一个可调用代码并返回一个``Future``对象; ``map () ``调度在当时多个异步调用,``shutdown()``
用来释放资源. 这个类是一个*context manager*,可以被用在``with``声明中,以确保当前pending futures运行完毕时资源被自动释放. 

A simple of example of :class:`~concurrent.futures.ThreadPoolExecutor` is a
launch of four parallel threads for copying files::

一个``ThreadPoolExecutor``的例子,运行了4个并发的复制文件的线程::
...
Python将字节码缓存在*. pyc文件*文件并不能很好的工作,特别是
有多个Python解释器环境时. 如果一名解释器
遇到了另一个解释器创建的缓存文件时,它会
重新编译源和覆盖已缓存的文件,这样就会失去
缓存的本意和优势. 
...
These conflicts also arise with CPython alternatives such as Unladen Swallow.
 ...
 "pyc文件相互打架" 的问题变得更加突出,因为它已成为
为多个Python同时存在的Linux各发行版本司空见惯现象
.这些冲突也出现于CPython的替代品,如
Unladen Swallow. 
...
为了解决这个问题,Python的import体系已打算
针对不同解释器使用不同的文件名. 代替Python 3.2
和Python 3.3,Unladen Swallow缓存冲突机制的方法是生成一个叫做
 "mymodule.pyc" 的文件,他们现在会寻找 "mymodule.cpython - 32.pyc" 
 "mymodule.cpython - 33.pyc" 和 "mymodule.unladen10.pyc" . 并且
为了防止这些新文件塞满源文档目录,
* pyc文件*文件现在会被存放在相应源文件包目录下的 "__pycache__" 目录.
...
* 导入的模块现在有一个属性``__cached__``, 它存储被导入文件的真实文件名::
...
* 每个解释器都有一个独一无二的标记,可以从"imp"模块来访问它"::
...
* 脚本从导入的文件推断源文件名现在需要更聪明的办法.
  它不再是简单地去掉"pyc"文件名里面的"c".
  相反,现在使用"imp"模块中的新函数来实现它::
...
* `` py_compile ``和`` compileall``模块的新更新
  反映了针对目标目录新的命名约定. 该
  命令行调用*compileall``* 时有了新的选项:"-i"表示
  将会被编译的指定的文件和目录列表,``-b``
  将导致字节码文件被写入到其传统位置
  而不是*__pycache__*.
...
* ``importlib.abc``模块伴随着装载字节码文件的新*抽象
  基类*一起被更新. 过时的ABCs,
  `` PyLoader `` 和`` PyPycLoader``,已被废除(
  附带了如何保持与Python 3.1兼容性的帮助文档.)
...
PYC 允许多个字节码缓存文件的文件目录
设在同一路径. 这篇PEP实现了一个类似的机制,共享
目标文件,针对不同语言版本,给他们一个共同的目录和不同的名称.
...
常见的目录是 "pyshared" ,文件名是由
通过识别不同的Python实现而生成的的 (如CPython的,
PyPy,Jython中,等) ,主要和次要版本号,以及可选
创建标记 (如 "d" 表示调试, "m" 代表pymalloc, "u" 为
多类别Unicode对于任选的包比如 "foo" ,你可能会在安装Python程序后看到这些文件::
...
Python本身的标记是通过``sysconfig `` 模块的函数进行访问的::
...
*Python PEP 3333: Python的Web服务器网关接口1.0.1
=====================================================
...
本PEP参考阐明了如何通过WGSI协议处理 字节/文字的细节问题.
目前的挑战是,字符串处理
在Python 3中最方便的方式是处理``str``类型,
虽然HTTP协议本身更擅长有针对性的处理特定的字节类型.
...
PEP专门区分了所谓的用于
请求/响应头的*native strings*和用于请求/响应体的元数据类型*byte strings*.
...
*native strings*总会是"str"类型,只是对于那些编码范围在*U+0000*和*U+00FF*之间的代码会使用*Latin-1*编码方式进行转换.
这些字符串是用于
表示响应报头环境字典的键和值或"start_response"函数的状态.
他们必须遵从
**RFC 2616**编码要求. 也就是说,它们必须是
*ISO-8859- 1*字符或是**RFC 2047**中的MIME编码. 
如果要从Python2的移植WSGI的应用,这里有几个
要点: 

*如果报头已经使用Python2中的字符串,应用程序无需改变.
是必要的. 

*如果相反,应用程序对输出报头进行了编码或对输入报头进行了解码,
则报头将需要重新以 Latin-1 方式编码. 
例如,一个输出报头用"h.encode('utf-8')"编码成UTF-8
则需要从字节转换为原生
字符串,且必须使用``h.encode('utf-8').decode('latin-1')``方式.

*应用程序生成的变量或使用``write () ``方式发送出去的字符串必须是byte strings.
`` start_response () ``函数
和与其交互的函数必须使用native strings.两者不能混用. 
...
为了方便开发者写 CGI-to-WSGI 或者类似的转换协议,
用户必须能访问该使用native strings的环境
,虽然同一个平台可能有
不同的约定. 为了弥补这一空白,``wsgiref`` 模块
有了一个新函数,`` wsgiref.handlers.read_environ () ``,可以将
CGI变量从`` os.environ``转换为native strings并且返回一个新字典.
...
* `` format () ``和`` str.format () 获得了使用**#**进行字符串格式化的新功能.
  此前, 整数的二进制,八进制或十六进制形式,都会分别有个'0b','0o','0x'前缀.
  现在,它也可以这样处理,浮点数,复数,十进制数,最终输出会有一个小数点,即使没有数字跟随它::
...
* 还有一个新的`` str.format_map () ``方法,扩展了现有的`` str.format () ``方法,可接受
  任意*mapping*映射对象. 这种新方法使我们能够对Python的许多类字典
  对象进行字符串格式化,如``defaultdict `` ,``Shelf``,``ConfigParser`` ,或
  "dbm".这对我们自定义一个在查询前先初始化正常化字典key的子类也很有帮助,
  或者提供一个`` __missing__ () ``方法处理未知keys::
...
* 解释器现在可以以安静模式启动,``-q``,可以阻止在交互模式中显示版权和版本信息.
  该选项可以被``sys.flags `` 属性内省::
...
* `` hasattr () ``函数通过调用 ``getattr () ``检测是否触发异常. 这种技术允许它
  检测`` __getattr__ () ``或`` __getattribute__ () ``动态创建的方法是否存在于类字典中.
  以前,* hasattr *会触发任何异常,可能掩盖了真正的错误. 现在,* hasattr *已被限制于
  只能扑捉到 ``AttributeError ``,其他异常会被它忽略:::
...
* 浮点数和复数使用"str()"的效果现在和"repr"是一样的.此前,``str () ``形式更短,不过这容易
  造成混乱,现在不再需要,``repr () ``将会是最短格式的默认显示: 
...
* ``memoryview`` 对象现在有一个``release () ``方法,他们也
  支持context manager协议. 这将允许在请求新的内存缓存时候及时释放已过时对象的资源:::
...
* 删除本地命名空间的名称以前它是非法的,尤其它作为自由变量出现在嵌套快中:::
  ...
  现在这是允许的. 请记住,``except``会导致清零,
  所以这种代码在Python 2.6中会工作,在Python3.1中会触发"SyntaxError"异常,,现在又被重新加入进3.2版本了:::
...
* 内部`` structsequence ``工具现在会创建一个元组的子集.
  这意味着像那些被"os.stat()","time.gmtime()","sys.version_info"返回的C结构体会如同一个*已命名元组*进行工作,
  并且会被其他方法或函数当做其"元组型参数",这是很大进步,将会使得C结构体和纯Python组件一样灵活便于使用:::
...
* ``PYTHONWARNINGS``环境变量现在更容易控制警告选项也可以在命令行中使用``-W``选项获得同样效果:::
...
* 一个新的警告类别,``ResourceWarning`` ,已添加. 这是
  它会在资源被耗光或被异常清楚时候发出警告,它默认是关闭的,但可以通过``warnings``模块启用,或通过命令行. 
  ...
  ``ResourceWarning``会在解释器被关闭但
  ``gc.garbage``列表并不为空的情况下被放出,若此时``gc.DEBUG_UNCOLLECTABLE``
  被设置,所有无法收回的对象将会被打印出来. 这是为了让程序员意识到他们的代码包含了未被明确决定归属的对象的现象.
...
`` ResourceWarning``在一个文件对象未被显式关闭却被突然销毁的情况下也会被放出,虽然deallocator会确保它关闭底层
  操作系统资源 (通常是一个文件描述符) ,在具体异常问题的延时时间里却可能发生各种可能,特别是在Windows下. 这里有一个的例子:::
...
* `` range``对象现在支持*index*和*count*方法. 这是使更多的对象全面落实
  "collections Sequence" *抽象基类*努力的一部分,.这样做的结果是这个语言将会有一个更加统一的API. 此外,``range``
  对象现在支持切片和nagative indices,包括其值大于"sys.maxsize"时也可以处理.这使得操作*range*对象时更类似于操作列表:::
...
* Py2.x中的内置函数``callable () ``被保留. 
  提供了一个简明,使用一个抽象基可读*替代在一类*喜欢`` isinstance (X表达,* collections可赎回) ``: 
...
* Python的导入机制现在可以加载那些目录路径名为非ASCII名的已安装模块.
  目录名与路径中的非ASCII字符. 这解决了日益严重的问题:用户使用非ASCII编码的路径名也可正常工作.
...
Python的3.2最大的新闻是``email``包,
``mailbox``模块,``nntplib``模块现在可以在
Python3下面的 byte/text 模式下正常工作. 这是首次有可以
完美处理messages中混合编码的方法. 
...
编码和文本字节的问题. 特别是,
与操作系统配合现在能够更好地处理
非ASCII数据比如使用使用Windows MBCS编码,locale-aware
编码,或UTF-8. 
...
此外,现在更多的类实现了*context manager*以支持使用"with"进行更彻底的资源清理.
...
*新函数`` message_from_bytes () ``和
`` message_from_binary_file ()``,新的类``BytesFeedParser``
和``BytesParser`` 允许二进制信息数据被解析成
模型对象. 
...
``threading``模块有一个新的同步类`` Barrier``,
可使多个线程处在等待状态,直到他们都已经达到了
常见的障碍点. Barriers在确认多个线程是否在前任线程任务完成前处在待命状态时十分有用.
...
作为一个两阶段实施循环障碍,障碍`` ``对象
适合于循环使用. 单独填写*和* * *排水
阶段得到保证所有线程前公布任何一个 (排水) 
他们可以回送,并重新进入障碍. 充分的屏障
复位后,每个周期. 
