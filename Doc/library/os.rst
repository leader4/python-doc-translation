...
.. exception:: error
   ...
   exception os.error 都是内建的``OSError``类型异常的别名
...
.. data:: name
   ...
   导入的独立于操作系统的模块的名字. 以下的这些名字已经被注册:``'posix'``,
	``'nt'`` ,``'os2'``,``'ce'``,``'java'``,``'riscos'``
...
.. data:: environ
   ...
   .. note::
      ...
      注: 直接调用``putenv()``不会改变``os.environ``,所以最好是直接改动
	``os.environ``
...
.. function:: chdir(path)
   ...
   这几个函数在*Files 和 Directories*中描述. 
   更改路径,查找路径,获得当前路径
...
.. function:: fsencode(filename)
   ...
   error handler, or ``'strict'`` on Windows; return :class:`bytes` unchanged.
     ...
     对"filename"进行系统级文件编码,使用"surrogateescape"错误处理器,在Windows平台使用"strict",返回的"bytes"不变.
...
.. function:: get_exec_path(env=None)
   ...
   *env*, when specified, should be an environment variable dictionary
   ...
   返回一个被查询可执行文件名字的路径列表,运行进程时类似于shell,当修改"env"时,应该用一个环境变量路径来用以确定查找范围.
   默认"env"是None时,"environ"正被使用.
...
.. function:: getegid()
   ...
   返回当前进程被激活的所有id.对应"set id".只在Unix下可用.
...
.. function:: initgroups(username, gid)
   ...
   调用system initgrops() 初始化group访问列表,所有的名字是数字的groups会被加上一个指定的group ID.
...
.. function:: getlogin()
   ...
   返回当前进程控制终端的登录用户名.大多数时候,可以使用环境变量"LOGNAME"或"USENAME"找出谁是用户,或者
   "pwd.getpwuid(os.getuid())[0]"获得当前被激活的登录用户ID.
...
.. function:: getpgid(pid)
   ...
   返回进程id是"pid"的进程组ID.若"pid"是0,当前进程组会被返回.
...
.. function:: getppid()
   ...
   返回父进程ID.若父进程存在,在Unix上该id会是初始进程(1)的其中一个,在Windows上它将是一个命名id,可能与其他进程重名.
...
.. function:: getresuid()
   ...
   返回一个元祖(ruid,euid,suid)对应当前进程的真实的,活动的,保存的用户id.
...
.. function:: getresgid()
   ...
   返回一个元祖(rgid,egid,sgid)对应当前进程的真实的,活动的,保存的用户ids.
...
.. function:: getenv(key, default=None)
   ...
   *default* if it doesn't. *key*, *default* and the result are str.
   ...
   返回环境变量key对应的值,key不存在时返回default对应的值,返回的都是字符串.
...
.. function:: getenvb(key, default=None)
   ...
   *default* if it doesn't. *key*, *default* and the result are bytes.
   ...
   Availability: most flavors of Unix.
      ...
      返回环境变量key对应的值,key不存在时返回default对应的值,返回的都是二进制值.
...
.. function:: putenv(key, value)
   ...
   Availability: most flavors of Unix, Windows.
     ...
     将key对应的字符串"value"设置为环境变量,以方便可以使用如下函数:
   ...
   .. note::
      ...
      在FreeBSD或Mac OS X上,设置"environ"可能会导致内存溢出.
...
.. function:: setgroups(groups)
   ...
   *groups*. *groups* must be a sequence, and each element must be an integer
   ...
   Availability: Unix.
    ...
    设置与当前进程相关联的"补充"组ids."groups"必须是一个序列,每一项必须是整数标识一个组.这个操作一般只提供给超级用户.
...
.. function:: setpgrp()
   ...
   调用系统级功能"setpgrp()",或者是"setpgrp(0,0)()",这取决于python的版本实现.
...
.. function:: setpgid(pid, pgrp)
   ...
   Availability: Unix.
     ...
     调用系统级功能"setpgid()"设置进程组id,将"pid"改成"pgrp".
...
.. function:: setresgid(rgid, egid, sgid)
   ...
   设置当前进程的real,effective,saved进程组id.
...
.. function:: setresuid(ruid, euid, suid)
   ...
   设置当前进程的real,effective,saved进程id.
...
.. function:: setreuid(ruid, euid)
   ...
   设置当前进程的real,effective,saved用户进程id.
...
.. function:: strerror(code)
   ...
   返回一个错误信息,对应"code"中的错误代码."strerror()"会返回NULL,当提供的参数是一个未知的错误数字,"ValueError"异常会被抛出.
...
.. data:: supports_bytes_environ
   ...
   返回True,如果系统环境支持bytes类型.(Windows会返回False)
...
.. function:: uname()
   ...
   ``socket.gethostbyaddr(socket.gethostname())``.
    ...
    返回一个5元元组,包含了当前操作系统的标识信息,"(sysname,nadename,release,version,machine)".
...
.. function:: fdopen(fd[, mode[, bufsize]])
   ...
   *O_APPEND*标志位会被置位. (大多数平台的``fopen()``函数实现也会这样做) . 
   ...
   在Unix上,mode 参数若使用a,"O_APPEND"标记会作用于文件描述器.
